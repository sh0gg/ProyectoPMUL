...
public class Televisor {
    ...
    public void cambiarCanal(Canal nuevoCanal) {
        if (nuevoCanal.getDisponibilidad().equals("PAGO")) {
            ...
            if (bar.getEnUso()[bar.getSuscritos().indexOf(nuevoCanal)]++ < bar.getSuscripciones().get(bar.getSuscritos().indexOf(nuevoCanal))) {
                bar.usarCanal(nuevoCanal);
                canalActual = nuevoCanal;
                estatus = "Esta viendo:" + canalActual.getNombre();
                int precioActual = canalActual.getCuota() + (canalActual.getPrecioPorPantalla() * bar.getSuscripciones().get(bar.getSuscritos().indexOf(canalActual)));
                detalles = "Pago actual = " + precioActual;
            } else {
                bar.usarCanal(nuevoCanal);
                canalActual = nuevoCanal;
                int precioActual = nuevoCanal.getCuota() + (canalActual.getPrecioPorPantalla() * bar.getSuscripciones().get(bar.getSuscritos().indexOf(nuevoCanal)));
                int precioNuevo = nuevoCanal.getCuota() + (nuevoCanal.getPrecioPorPantalla() * (bar.getSuscripciones().get(bar.getSuscritos().indexOf(nuevoCanal)) + 1));
