...
public class MainActivity extends AppCompatActivity {
    ...
    private RecyclerView rvOptionEdit, rvPolls;
    private Button btnAddOption, btnCreate, btnStartLatest;
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        rvOptionEdit = findViewById(R.id.rvOptionEdit);
        rvPolls = findViewById(R.id.rvPolls);
        btnAddOption = findViewById(R.id.btnAddOption);
        btnCreate = findViewById(R.id.btnCreate);
        btnStartLatest = findViewById(R.id.btnStartLatest);
        ...
        pollListAdapter = new PollListAdapter(new PollListAdapter.Listener() {
            @Override public void onStart(Poll p) { startPoll(p); }
            @Override public void onDelete(Poll p) { repo.delete(p.getId());
                refreshPolls(); }
            @Override public void onEdit(Poll p) {
        ...
        btnCreate.setOnClickListener(v -> {
            ...
            { etTitle.setError(getString(R.string.err_required)); return; }
            if (TextUtils.isEmpty(votersStr))
            { etVoters.setError(getString(R.string.err_required)); return; }
            int voters = 0;
            try { voters = Integer.parseInt(votersStr); } catch (Exception
                    ignored) {}
            if (voters < 1) { etVoters.setError(getString(R.string.err_voters));
                return; }
            int nonEmpty = 0;
            for (String t : optionTexts) if (t != null && !t.trim().isEmpty())
                nonEmpty++;
            if (nonEmpty < 2) { Toast.makeText(this, R.string.err_min_options,
    ...
    private void refreshPolls() { pollListAdapter.submit(repo.getAll()); }
