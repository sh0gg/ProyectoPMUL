...
public class PollRepository {
    ...
    public ArrayList<Poll> getAll() { return new ArrayList<>(cache); }
    public Poll getCurrent() { return current; }
    public void setCurrent(Poll p) { current = p; }
    public Poll create(String title, int expected, ArrayList<String>
            optionTexts) {
    ...
    public void delete(String id) {
        for (int i = 0; i < cache.size(); i++) {
            if (cache.get(i).getId().equals(id)) { cache.remove(i); break; }
        }
